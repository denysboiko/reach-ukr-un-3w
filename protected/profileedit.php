<?php
	...
	class ProfileEditForm extends Form {
		function init() {
			...
			$this->notification_receiver = new BooleanField([ 'label' => new Label(' notify me on new users registration '), 'required' => false]);
	...
	if($method == 'GET') {
		...
		$form->notification_receiver->value = $session->user->notification_receiver;
	} else if ($method == 'POST') {
		...
		if($form->is_valid) {
			...
			if($user->checkRole(['admin'])) {
				$user->notification_receiver = $form->notification_receiver->value;
			}
